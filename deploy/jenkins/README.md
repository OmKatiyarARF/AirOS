...
| `pipeline_file` | no | Path **relative to the root of the AirOS repo** of an AirOS-owned Jenkinsfile, e.g. `deploy/jenkins/pipelines/dss-backend-modular-test.Jenkinsfile`. When set, creates a **standalone** Pipeline job defined as *Pipeline script from SCM*: Jenkins fetches that file from the AirOS repo (URL/branch/credential taken from the `AirOS` entry) on every build, instead of a multibranch job reading the target repo's own `Jenkinsfile`. Use this for jobs that must not touch the app repo (isolated test-environment deploys, etc.) — the script is self-contained (its own checkout + `pollSCM` trigger), so `branch`/`git_url` become informational only |
| `sandbox` | no | **Obsolete and ignored.** A *Pipeline script from SCM* definition has no sandbox flag: Jenkins runs SCM-sourced pipelines trusted and exempt from Script Approval, which is what the `@NonCPS` rollback jobs need. Guard these scripts with review/branch protection on the AirOS repo, not with Jenkins config |
